...
function rebuildClocks() {
  ...
  timezones.forEach(function(timezone, index) {
    ...
    removeTimezoneButton.addEventListener('click', function() {
  ...
  timezones.forEach(function(timezone, i) {
    var dateFormat = NewDateTimeFormat({
      timeZone: timezone
    });
    var dateString = dateFormat.format(date);
    var timeZoneFormat = NewDateTimeFormat({
      timeZoneName: 'long',
      timeZone: timezone
    });
  ...
  /* Evesham Update - WC-314 */
  document.getElementById("clock0").style.opacity = 1;
...
function makeCalendar() {
  ...
  for (var d = 1; d <= 7; d++) {
    ...
    var wd = date.getDay();
    days[wd] = NewDateTimeFormat({
      weekday: 'long'
    }).format(date);
    daysShort[wd] = NewDateTimeFormat({
      weekday: 'short'
    }).format(date);
  }
  for (var m = 0; m < 12; m++) {
    date.setFullYear(2012, m, 1);
    months.push(NewDateTimeFormat({
      month: 'long'
    }).format(date));
    monthsShort.push(NewDateTimeFormat({
      month: 'short'
    }).format(date));
  }
...
function timerTick() {
  var now = new Date();
  timezones.forEach(function(timezone, i) {
    var hours = NewDateTimeFormat({
      hour: '2-digit',
      hour12: false,
      timeZone: timezone
    }).format(now);
    var minutes = Number.parseInt(NewDateTimeFormat({
      minute: '2-digit',
      timeZone: timezone
    }).format(now));
    ...
    document.querySelector('#clock' + i + ' .time').textContent =
      now.toLocaleTimeString(undefined, {
        timeZone: getChromeTimeZone(timezone)
      });
